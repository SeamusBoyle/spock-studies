...
obj.exec = function (period) {
  ...
  const maxLookback = Math.max(obj._spanBPeriods.value, obj._kijunPeriods.value, obj._tenkenPeriods.value);
  if (period < maxLookback)
    return;

  const highs = Plot.high(period, maxLookback);
  const lows = Plot.low(period, maxLookback);

  const tenkenSen = (Plot.max(highs, obj._tenkenPeriods.value) + Plot.min(lows, obj._tenkenPeriods.value)) / 2;

  const kijunSen = (Plot.max(highs, obj._kijunPeriods.value) + Plot.min(lows, obj._kijunPeriods.value)) / 2;

  const spanA = (tenkenSen + kijunSen) / 2;

  const spanB = (Plot.max(highs, obj._spanBPeriods.value) + Plot.min(lows, obj._spanBPeriods.value)) / 2;

  const chikou = Plot.close(period);
