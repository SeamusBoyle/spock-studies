...
obj.exec = function (period) {
  ...
  var shiftedJaw = obj._jaw[period - obj._jawShift.value]
  var shiftedTeeth = obj._teeth[period - obj._teethShift.value]
  var shiftedLips = obj._lips[period - obj._lipsShift.value]

  obj._upper[period] = Math.abs(shiftedJaw - shiftedTeeth)
  obj._lower[period] = -Math.abs(shiftedTeeth - shiftedLips)

  var upperColor
  var prevUpper = obj._upper[period - 1]
  if (prevUpper != null) {
  ...
  var lowerColor
  var prevLower = obj._lower[period - 1]
  if (prevLower != null) {
